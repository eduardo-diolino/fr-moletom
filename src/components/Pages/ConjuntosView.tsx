...
export default function ConjuntosView({
  products,
  favorites,
  onViewProduct,
  onAddToCart,
  ...
  // Filter products for category 'conjuntos'
  const categoryProducts = products.filter(
    (p) => p.category === 'conjuntos'
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 animate-fadeIn">
      {/* Category Banner */}
      <div className="relative bg-gradient-to-r from-orange-900 to-orange-750 text-white p-8 md:p-12 mb-10 overflow-hidden shadow-sm border border-orange-850">
        <div className="absolute inset-0 bg-black/20" />
        <div className="absolute right-0 bottom-0 translate-y-12 translate-x-12 opacity-11 font-black text-9xl">
          CONJUNTOS
        </div>
        <div className="relative z-10 max-w-xl">
          <span className="bg-[#002855] text-white text-[10px] font-black uppercase tracking-widest px-2.5 py-1 inline-block mb-3 select-none">
            Combinações Estilosas Completas
          </span>
          ...
          <p className="text-xs sm:text-sm text-orange-100 font-medium leading-relaxed">
            Nossos combos premium: blusão com capuz + calças jogger ou t-shirt nobre + shorts bermuda. Facilidade máxima na hora de vestir seu menino com looks combinando que encantam!
          </p>
        </div>
      </div>
      ...
      {filteredProducts.length === 0 ? (
        <div className="bg-white border border-gray-100 p-16 text-center shadow-xs">
          ...
          <p className="text-xs text-gray-400 font-bold max-w-sm mx-auto mb-6">
            Nenhum conjunto correspondente foi localizado para os filtros ativos no momento.
          </p>
          <button
            onClick={() => {
