...
import { ToggleLeft, ToggleRight, Grid, Filter, Check } from 'lucide-react';

interface CalcasViewProps {
  products: Product[];
  favorites: string[];
...
export default function CalcasView({
  products,
  favorites,
  onViewProduct,
  onAddToCart,
  ...
  // Filter products for Calças category only
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 animate-fadeIn">
      {/* Category Banner */}
      <div className="relative bg-gradient-to-r from-amber-900 to-amber-750 text-white p-8 md:p-12 mb-10 overflow-hidden shadow-sm border border-amber-800">
        ...
        <div className="relative z-10 max-w-xl">
          <span className="bg-[#FF7A00] text-white text-[10px] font-black uppercase tracking-widest px-2.5 py-1 inline-block mb-3 select-none">
            Inverno Quentinho & Confortável
          </span>
          <h1 className="text-3xl sm:text-5xl font-extrabold font-display uppercase tracking-wider mb-2">
            Calças de Moletom
          </h1>
          <p className="text-xs sm:text-sm text-amber-100 font-medium leading-relaxed">
            Calças jogger leves, forradas e flaneladas de altíssima gramatura. Proteção térmica, design moderno europeu e conforto elástico sob medida para as brincadeiras e o dia a dia!
          </p>
        </div>
      </div>
      ...
      {filteredProducts.length === 0 ? (
        <div className="bg-white border border-gray-100 p-16 text-center shadow-xs">
          ...
          <p className="text-xs text-gray-400 font-bold max-w-sm mx-auto mb-6">
            Nenhuma calça no estoque corresponde exatamente aos filtros selecionados por enquanto.
          </p>
          <button
            onClick={() => {
